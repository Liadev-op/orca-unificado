#include "OrcaCloudLoginDialog.hpp"
...
BEGIN_EVENT_TABLE(OrcaCloudLoginDialog, wxDialog)
EVT_TIMER(NETWORK_OFFLINE_TIMER_ID, OrcaCloudLoginDialog::OnTimer)
END_EVENT_TABLE()

int OrcaCloudLoginDialog::web_sequence_id = 20000;
    ...
    : wxDialog((wxWindow*) (wxGetApp().mainframe), wxID_ANY, "Orca Cloud"), m_cloud_agent(cloud_agent)
{
    ...
    m_user_agent = wxString::Format("OrcaUnificado/v%s", SLIC3R_VERSION);
    ...
    Bind(wxEVT_WEBVIEW_NAVIGATING, &OrcaCloudLoginDialog::OnNavigationRequest, this, m_browser->GetId());
    Bind(wxEVT_WEBVIEW_NAVIGATED, &OrcaCloudLoginDialog::OnNavigationComplete, this, m_browser->GetId());
    Bind(wxEVT_WEBVIEW_LOADED, &OrcaCloudLoginDialog::OnDocumentLoaded, this, m_browser->GetId());
    Bind(wxEVT_WEBVIEW_ERROR, &OrcaCloudLoginDialog::OnError, this, m_browser->GetId());
    Bind(wxEVT_WEBVIEW_NEWWINDOW, &OrcaCloudLoginDialog::OnNewWindow, this, m_browser->GetId());
    Bind(wxEVT_WEBVIEW_TITLE_CHANGED, &OrcaCloudLoginDialog::OnTitleChanged, this, m_browser->GetId());
    Bind(wxEVT_WEBVIEW_FULLSCREEN_CHANGED, &OrcaCloudLoginDialog::OnFullScreenChanged, this, m_browser->GetId());
...
OrcaCloudLoginDialog::~OrcaCloudLoginDialog() {
    if (m_timer != NULL) {
        m_timer->Stop();
        delete m_timer;
        m_timer = NULL;
...
void OrcaCloudLoginDialog::OnTimer(wxTimerEvent &event) {
    m_timer->Stop();
...
void OrcaCloudLoginDialog::load_url(wxString &url)
{
 ...
 */
void OrcaCloudLoginDialog::UpdateState()
{
    // SetTitle(m_browser->GetCurrentTitle());
}

void OrcaCloudLoginDialog::OnIdle(wxIdleEvent &WXUNUSED(evt))
{
    if (m_browser->IsBusy()) {
...
// void OrcaCloudLoginDialog::OnClose(wxCloseEvent& evt)
//{
//    this->Hide();
//}
 ...
 */
void OrcaCloudLoginDialog::OnNavigationRequest(wxWebViewEvent &evt)
{
 ...
 */
void OrcaCloudLoginDialog::OnNavigationComplete(wxWebViewEvent &evt)
{
 ...
 */
void OrcaCloudLoginDialog::OnDocumentLoaded(wxWebViewEvent &evt)
{
 ...
 */
void OrcaCloudLoginDialog::OnNewWindow(wxWebViewEvent &evt)
{
    wxString flag = " (other)";
...
void OrcaCloudLoginDialog::OnTitleChanged(wxWebViewEvent &evt)
{
    // SetTitle(evt.GetString());
    // wxLogMessage("%s", "Title changed; title='" + evt.GetString() + "'");
}

void OrcaCloudLoginDialog::OnFullScreenChanged(wxWebViewEvent &evt)
{
    // wxLogMessage("Full screen changed; status = %d", evt.GetInt());
    ShowFullScreen(evt.GetInt() != 0);
}

void OrcaCloudLoginDialog::OnScriptMessage(wxWebViewEvent &evt)
{
    ...
    try {
        ...
        if (strCmd == "user_ticket_login") {
            if (!m_cloud_agent || !j.contains("data") || !j["data"].is_object() || !j["data"].contains("ticket")) {
                wxMessageBox(_L("Login failed. Please try again."), _L("Orca Cloud"), wxICON_WARNING);
                return;
            }
            ...
            int          token_result = m_cloud_agent->get_my_token(ticket, &token_http_code, &token_body);
            if (token_result != 0) {
            ...
            int          profile_result = m_cloud_agent->get_my_profile(access_token, &profile_http_code, &profile_body);
            if (profile_result != 0) {
            ...
            wxTheApp->CallAfter([message_json]() { wxGetApp().handle_orca_cloud_script_message(message_json); });
        }
...
void OrcaCloudLoginDialog::RunScript(const wxString &javascript)
{
...
#if wxUSE_WEBVIEW_IE
void OrcaCloudLoginDialog::OnRunScriptObjectWithEmulationLevel(wxCommandEvent &WXUNUSED(evt))
{
...
void OrcaCloudLoginDialog::OnRunScriptDateWithEmulationLevel(wxCommandEvent &WXUNUSED(evt))
{
    wxWebViewIE::MSWSetModernEmulationLevel();
...
void OrcaCloudLoginDialog::OnRunScriptArrayWithEmulationLevel(wxCommandEvent &WXUNUSED(evt))
{
    wxWebViewIE::MSWSetModernEmulationLevel();
 ...
 */
void OrcaCloudLoginDialog::OnError(wxWebViewEvent &evt)
{
...
void OrcaCloudLoginDialog::OnScriptResponseMessage(wxCommandEvent &WXUNUSED(evt))
{
...
bool  OrcaCloudLoginDialog::ShowErrorPage()
{
